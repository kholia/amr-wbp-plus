...
MP4_EXTERN( MP4Err )
MP4AddMediaSampleReferencePad( MP4Media media, u64 dataOffset, u32 sampleCount, MP4Handle durationsH, MP4Handle sizesH, MP4Handle sampleEntryH, MP4Handle decodingOffsetsH, MP4Handle syncSamplesH, MP4Handle padsH )
{
	MP4Err          err;
	MP4MediaAtomPtr mdia;
	...
	err = mdia->addSampleReference( mdia, dataOffset, sampleCount, durationsH, sizesH, sampleEntryH, decodingOffsetsH, syncSamplesH, padsH );
...
MP4_EXTERN( MP4Err )
MP4AddMediaSamplesPad( MP4Media media, MP4Handle sampleH, u32 sampleCount, MP4Handle durationsH, MP4Handle sizesH, MP4Handle sampleEntryH, MP4Handle decodingOffsetsH, MP4Handle syncSamplesH, MP4Handle padsH )
{
	MP4Err          err;
	MP4MediaAtomPtr mdia;
	...
	mdia = (MP4MediaAtomPtr)media;
	err = mdia->addSamples( mdia, sampleH, sampleCount, durationsH, sizesH, sampleEntryH, decodingOffsetsH, syncSamplesH, padsH );
...
MP4_EXTERN( MP4Err )
MP4AddMediaSampleReference( MP4Media media, u64 dataOffset, u32 sampleCount, MP4Handle durationsH, MP4Handle sizesH, MP4Handle sampleEntryH, MP4Handle decodingOffsetsH, MP4Handle syncSamplesH )
{
	MP4Err          err;
	MP4MediaAtomPtr mdia;
	...
	mdia = (MP4MediaAtomPtr)media;
	err = mdia->addSampleReference( mdia, dataOffset, sampleCount, durationsH, sizesH, sampleEntryH, decodingOffsetsH, syncSamplesH, NULL );
...
MP4_EXTERN( MP4Err )
MP4AddMediaSamples( MP4Media media, MP4Handle sampleH, u32 sampleCount, MP4Handle durationsH, MP4Handle sizesH, MP4Handle sampleEntryH, MP4Handle decodingOffsetsH, MP4Handle syncSamplesH )
{
	MP4Err          err;
	MP4MediaAtomPtr mdia;
	...
	mdia = (MP4MediaAtomPtr)media;
	err = mdia->addSamples( mdia, sampleH, sampleCount, durationsH, sizesH, sampleEntryH, decodingOffsetsH, syncSamplesH, NULL );
...
MP4_EXTERN( MP4Err )
ISOSetSampleSizeField( MP4Media media, u32 fieldsize )
{
	MP4Err          err;
	MP4MediaAtomPtr mdia;
...
MP4_EXTERN( MP4Err )
MP4GetMediaDecoderType( MP4Media theMedia, u32 sampleDescIndex, u32 *outObjectType, u32 *outStreamType, u32 *outBufferSize, MP4Handle specificInfoH )
{
	return MP4GetMediaDecoderInformation( theMedia, sampleDescIndex, outObjectType, outStreamType, outBufferSize, NULL, /* upstream */
	    NULL,                                                                                                           /* max */
	    NULL,                                                                                                           /* avg */
	    specificInfoH );
}
...
)
{
	...
	err = stts->findSamples( stbl->TimeToSample, searchFromTime, &priorSample, &exactSample, &nextSample, &sampleNumber, &sampleDuration );
	if( err )
		goto bail;
...
MP4Err MP4GetMediaSampleDescIndex( MP4Media theMedia, u64 desiredTime, u32 *outSampleDescriptionIndex )
{
	...
	err = stts->findSamples( stbl->TimeToSample, desiredTime, &priorSample, &exactSample, &nextSample, &sampleNumber, &sampleDuration );
	if( err )
		goto bail;

	/* get sample description */
	err = stsc->lookupSample( stbl->SampleToChunk, sampleNumber, &chunkNumber, &sampleDescriptionIndex, &firstSampleNumberInChunk );
	if( err )
		goto bail;
...
MP4_EXTERN( MP4Err )
MP4GetMediaSample( MP4Media theMedia, MP4Handle outSample, u32 *outSize, u64 desiredDecodingTime, u64 *outDecodingTime, u64 *outCompositionTime, u64 *outDuration, MP4Handle outSampleDescription, u32 *outSampleDescriptionIndex, u32 *outSampleFlags )
{
	return MP4GetMediaSampleWithPad( theMedia, outSample, outSize, desiredDecodingTime, outDecodingTime, outCompositionTime, outDuration, outSampleDescription, outSampleDescriptionIndex, outSampleFlags, NULL );
}

MP4_EXTERN( MP4Err )
MP4GetMediaSampleWithPad( MP4Media theMedia, MP4Handle outSample, u32 *outSize, u64 desiredDecodingTime, u64 *outDecodingTime, u64 *outCompositionTime, u64 *outDuration, MP4Handle outSampleDescription, u32 *outSampleDescriptionIndex, u32 *outSampleFlags, u8 *outPad )
{
	MP4Err                      err;
	MP4MediaInformationAtomPtr  minf;
	MP4SampleTableAtomPtr       stbl;
	...
	err = stts->findSamples( stbl->TimeToSample, desiredDecodingTime, &priorSample, &exactSample, &nextSample, &sampleNumber, &sampleDuration );
	if( err )
		goto bail;
	...
	/* get sample size */
	err = stsz->getSampleSizeAndOffset( stbl->SampleSize, sampleNumber, &sampleSize, firstSampleNumberInChunk, &sampleOffsetWithinChunk );
	if( err )
		goto bail;
	*outSize = sampleSize;
	if( outPad ) {
	...
	/* get the sample */
	err = MP4GetMediaSampleDescription( theMedia, sampleDescriptionIndex, outSampleDescription, &dataReferenceIndex );
	if( err )
		goto bail;
    ...
    u32 *                      outSampleDescIndex )
{
	return MP4GetIndMediaSampleWithPad(
	    theMedia, sampleNumber, outSample, outSize, outDTS, outCTSOffset, outDuration, outSampleFlags, outSampleDescIndex, NULL );
}
    ...
    MP4Handle                           sampleDesc )
{
	...
	*outSize = sampleSize;
	...
	err = MP4GetMediaSampleDescription( theMedia, sampleDescriptionIndex, sampleDesc, &dataReferenceIndex );
	if( err )
		goto bail;
...
MP4MediaTimeToSampleNum( MP4Media theMedia, u64 mediaTime, u32 *outSampleNum, u64 *outSampleCTS, u64 *outSampleDTS, s32 *outSampleDuration )
{
	...
	/* get sample number and time */
	sampleNumber = 0;
	err = stts->findSamples( stbl->TimeToSample, mediaTime, &priorSample, &exactSample, &nextSample, &sampleNumber, &sampleDuration );
	if( err )
		goto bail;
	...
	if( exactSample != -1 )
		...
		if( priorSample != -1 )
			...
			sampleDTS = nextSample;
		err = stts->findSamples( stbl->TimeToSample, sampleDTS, &priorSample, &exactSample, &nextSample, &sampleNumber, &sampleDuration );
		if( err )
			goto bail;
		if( ( sampleDTS == -1 ) || ( sampleNumber == 0 ) )
...
MP4_EXTERN( MP4Err )
MP4SetupSampleAuxiliaryInformation( MP4Media theMedia, u8 isUsingAuxInfoPropertiesFlag, u32 aux_info_type, u32 aux_info_type_parameter, u8 default_sample_info_size )
{
	MP4Err                                      err;
	MP4MediaAtomPtr                             mdia;
	MP4MediaInformationAtomPtr                  minf;
	...
	err = stbl->getSampleAuxiliaryInformation( stbl, isUsingAuxInfoPropertiesFlag, aux_info_type, aux_info_type_parameter, &saizExisting, &saioExisting );
	if( err )
		goto bail;
...
MP4_EXTERN( MP4Err )
MP4AddSampleAuxiliaryInformation( MP4Media theMedia, u8 isUsingAuxInfoPropertiesFlag, u32 aux_info_type, u32 aux_info_type_parameter, MP4Handle dataH, u32 sampleCount, MP4Handle sizesH )
{
	MP4Err                                      err;
	MP4MediaAtomPtr                             mdia;
	MP4MediaInformationAtomPtr                  minf;
	...
	if( mdia->information->type == MP4MediaInformationAtomType ) {
		...
		err = stbl->getSampleAuxiliaryInformation( stbl, isUsingAuxInfoPropertiesFlag, aux_info_type, aux_info_type_parameter, &saiz, &saio );
		if( err )
			goto bail;
	}
		...
		err = traf->getSampleAuxiliaryInfoFromTrackFragment( traf, isUsingAuxInfoPropertiesFlag, aux_info_type, aux_info_type_parameter, &saiz, &saio );
		if( err )
			goto bail;
	}
...
MP4_EXTERN( MP4Err )
MP4GetSampleAuxiliaryInformation( MP4Media theMedia, u32 *outCount, MP4Handle isUsingAuxInfoPropertiesFlags, MP4Handle aux_info_types, MP4Handle aux_info_type_parameters )
{
	MP4Err                                    err;
	MP4MediaAtomPtr                           mdia;
	MP4MediaInformationAtomPtr                minf;
	...
	*outCount = 0;
	...
	err = MP4SetHandleSize( isUsingAuxInfoPropertiesFlags, stbl->SampleAuxiliaryInformationSizes->entryCount * sizeof( u8 ) );
	if( err )
		goto bail;
	err = MP4SetHandleSize( aux_info_types, stbl->SampleAuxiliaryInformationSizes->entryCount * sizeof( u32 ) );
	if( err )
		goto bail;
	err = MP4SetHandleSize( aux_info_type_parameters, stbl->SampleAuxiliaryInformationSizes->entryCount * sizeof( u32 ) );
	if( err )
		goto bail;
...
MP4GetSampleAuxiliaryInformationForSample( MP4Media theMedia, u8 isUsingAuxInfoPropertiesFlag, u32 aux_info_type, u32 aux_info_type_parameter, u32 sampleNr, MP4Handle outDataH, u32 *outSize )
{
	...
	*outSize = 0;
	...
	if( saiz->sample_count < sampleNr )
		BAILWITHERROR( MP4EOF );

	err = stsc->lookupSample( stbl->SampleToChunk, sampleNr, &chunkNumber, &sampleDescriptionIndex, &firstSampleNumberInChunk );
	if( err )
		goto bail;
