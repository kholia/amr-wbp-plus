...
bail:
	if( tool )
		tool->destroy( (MP4DescriptorPtr)tool );
	return err;
}
...
bail:
	if( desc )
		desc->destroy( (MP4DescriptorPtr)desc );
	return err;
}
...
bail:
	if( toolList )
		toolList->destroy( (MP4DescriptorPtr)toolList );
	return err;
}
...
bail:
	if( toolListDesc )
		toolListDesc->destroy( (MP4DescriptorPtr)toolListDesc );
	return err;
}
...
MP4Err MP4NewIPMPToolDescriptorUpdate( MP4Handle ipmpToolDescrUpdateH )
{
	...
	err = cmd->serialize( (MP4DescriptorPtr)cmd,
	    *ipmpToolDescrUpdateH );
	if( err )
		goto bail;
bail:
	if( cmd )
		cmd->destroy( (MP4DescriptorPtr)cmd );
	return err;
}
