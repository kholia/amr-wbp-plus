...
*/
		...
		err = MP4CreateDataHandler( self->inputStream, dataEntryAtom, (struct MP4DataHandler **)&self->dataHandler );
		if( err )
			goto bail;
...
static MP4Err addSampleReference( struct MP4MediaInformationAtom *self, u64 dataOffset, u32 sampleCount, MP4Handle durationsH, MP4Handle sizesH, MP4Handle sampleEntryH, MP4Handle decodingOffsetsH, MP4Handle syncSamplesH, MP4Handle padsH )
{
	MP4Err                    err;
	...
	if( sampleEntryH != NULL ) {
		...
		err = MP4ParseAtomUsingProtoList( is, MP4SampleEntryProtos, MP4GenericSampleEntryAtomType, &entry );
		if( err )
			goto bail;
	...
	err = stbl->addSamples( stbl, sampleCount, dataOffset, durationsH, sizesH, decodingOffsetsH, syncSamplesH, padsH );
	if( err )
		goto bail;
...
static MP4Err addSamples( struct MP4MediaInformationAtom *self, MP4Handle sampleH, u32 sampleCount, MP4Handle durationsH, MP4Handle sizesH, MP4Handle sampleEntryH, MP4Handle decodingOffsetsH, MP4Handle syncSamplesH, MP4Handle padsH )
{
	MP4Err                    err;
	u64                       sampleOffset;
	u32                       dataReferenceIndex;
	...
	if( sampleCount > 0 ) {
		...
		if( err )
			goto bail;
		err = stbl->addSamples( stbl, sampleCount, sampleOffset, durationsH, sizesH, decodingOffsetsH, syncSamplesH, padsH );
		if( err )
			goto bail;
