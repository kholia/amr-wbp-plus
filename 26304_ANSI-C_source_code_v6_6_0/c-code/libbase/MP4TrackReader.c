...
{
	return MP4TrackReaderGetNextAccessUnitWithDuration( theReader, outAccessUnit, outSize, outSampleFlags, outCTS, outDTS, 0 );
}

MP4_EXTERN( MP4Err )
MP4TrackReaderGetNextAccessUnitWithDuration( MP4TrackReader theReader, MP4Handle outAccessUnit, u32 *outSize, u32 *outSampleFlags, s32 *outCTS, s32 *outDTS, u32 *outDuration )
{
	MP4Err            err;
	MP4TrackReaderPtr reader;
	...
	reader = (MP4TrackReaderPtr)theReader;
	*outSize = 0;
	err = reader->getNextAccessUnit( reader, outAccessUnit, outSize, outSampleFlags, outCTS, outDTS, outDuration, 0 );
	if( err )
		goto bail;
bail:
	return err;
...
MP4_EXTERN( MP4Err )
MP4TrackReaderGetNextAccessUnitWithPad( MP4TrackReader theReader, MP4Handle outAccessUnit, u32 *outSize, u32 *outSampleFlags, s32 *outCTS, s32 *outDTS, u8 *outPad )
{
	MP4Err            err;
	MP4TrackReaderPtr reader;
	...
	reader = (MP4TrackReaderPtr)theReader;
	*outSize = 0;
	err = reader->getNextAccessUnit( reader, outAccessUnit, outSize, outSampleFlags, outCTS, outDTS, 0, outPad );
	if( err )
		goto bail;
bail:
	return err;
