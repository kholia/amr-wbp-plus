...
static MP4Err addSampleReference( struct MP4MediaAtom *self, u64 dataOffset, u32 sampleCount, MP4Handle durationsH, MP4Handle sizesH, MP4Handle sampleEntryH, MP4Handle decodingOffsetsH, MP4Handle syncSamplesH, MP4Handle padsH )
{
	MP4Err                     err;
	MP4MediaInformationAtomPtr minf;
	...
	err = minf->addSampleReference( minf, dataOffset, sampleCount, durationsH, sizesH, sampleEntryH, decodingOffsetsH, syncSamplesH, padsH );
	if( err )
		goto bail;
bail:
	TEST_RETURN( err );
...
static MP4Err settrackfragment( struct MP4MediaAtom *self, MP4AtomPtr fragment )
{
	...
	if( self->true_minf == NULL )
		self->true_minf = self->information;
...
static MP4Err addSamples( struct MP4MediaAtom *self, MP4Handle sampleH, u32 sampleCount, MP4Handle durationsH, MP4Handle sizesH, MP4Handle sampleEntryH, MP4Handle decodingOffsetsH, MP4Handle syncSamplesH, MP4Handle padsH )
{
	MP4Err                     err;
	MP4MediaInformationAtomPtr minf;
   ...
   */
	if( ( sampleCount > 0 ) && ( ( (MP4HandlerAtomPtr)self->handler )->handlerType == MP4ObjectDescriptorHandlerType ) ) {
	...
	err = minf->addSamples( minf, sampleH, sampleCount, durationsH, sizesH, sampleEntryH, decodingOffsetsH, syncSamplesH, padsH );
	if( err )
		goto bail;
bail:
	TEST_RETURN( err );
