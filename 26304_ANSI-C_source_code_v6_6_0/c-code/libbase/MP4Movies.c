...
MP4_EXTERN( MP4Err )
MergeMovieFragments( MP4PrivateMovieRecordPtr movie );
MP4_EXTERN( MP4Err )
MergeMovieData( MP4PrivateMovieRecordPtr movie );
...
MergeMovieData( MP4PrivateMovieRecordPtr movie )
{
	...
	if( movie->mdatList != NULL ) {
		...
		for( i = 0; i < mdatCount; i++ ) {
			...
			if( moov ) {
				err = moov->mdatMoved( moov, offset, secondary_mdat->dataSize + offset, ( s32 )( mdat->dataSize - offset ) );
			}
			if( meta ) {
				err = meta->mdatMoved( meta, offset, secondary_mdat->dataSize + offset, ( s32 )( mdat->dataSize - offset ) );
			}
...
MP4OpenMovieFile( MP4Movie *theMovie, const char *movieURL, int openMovieFlags )
{
	...
	moov->prepend_handle = NULL;
	moov->moovAtomPtr = NULL;
	err = MP4CreateFileMappingObject( (char *)movieURL, (struct FileMappingObjectRecord **)&moov->fileMappingObject );
	if( err )
		goto bail;
...
MP4SetMovieIODInlineProfileFlag( MP4Movie theMovie, u8 theFlag )
{
	...
	/* jlf 12/00: escape sequence for OD */
	if( ( iod->tag == MP4_OD_Tag ) || ( iod->tag == MP4ObjectDescriptorTag ) )
		return MP4HasRootOD;
...
{
	return MP4NewMovieExt( outMovie, initialODID, OD_profileAndLevel, scene_profileAndLevel, audio_profileAndLevel, visual_profileAndLevel, graphics_profileAndLevel, "", ( u8 )( ( ( OD_profileAndLevel == 0 ) && ( scene_profileAndLevel == 0 ) && ( audio_profileAndLevel == 0 ) && ( visual_profileAndLevel == 0 ) && ( graphics_profileAndLevel == 0 ) ) ? 1 : 0 ) );
