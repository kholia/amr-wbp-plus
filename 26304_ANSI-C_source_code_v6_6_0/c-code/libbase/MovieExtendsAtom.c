...
static MP4Err setCompositionToDecodeProperties( struct MP4MovieExtendsAtom *self, u32 trackID, s32 compositionToDTSShift, s32 leastDecodeToDisplayDelta, s32 greatestDecodeToDisplayDelta, s32 compositionStartTime, s32 compositionEndTime )
{
	MP4Err                             err;
...
static MP4Err setSampleDescriptionIndexes( struct MP4MovieExtendsAtom *self, MP4AtomPtr moov )
{
	...
	for( i = 0; i < trackCount; i++ ) {
		...
		err = ( (MP4MovieAtomPtr)moov )->getSampleDescriptionIndex( (MP4MovieAtom *)moov, trex->trackID, &sd_index );
		if( err )
			goto bail;
