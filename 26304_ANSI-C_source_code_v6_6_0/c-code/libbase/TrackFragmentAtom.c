...
static MP4Err mdatMoved( struct MP4MediaInformationAtom *s, u64 mdatBase, u64 mdatEnd, s32 mdatOffset )
{
	...
	if( self->samples_use_mdat == 1 )
		tfhd->base_data_offset = mdatOffset;
...
static MP4Err addSamples( struct MP4MediaInformationAtom *s, MP4Handle sampleH, u32 sampleCount, MP4Handle durationsH, MP4Handle sizesH, MP4Handle sampleEntryH, MP4Handle decodingOffsetsH, MP4Handle syncSamplesH, MP4Handle padsH )
{
	...
	if( ( self->samples_use_mdat == 0 ) || ( self->samples_use_mdat == 1 ) )
		self->samples_use_mdat = 1;
	else
		BAILWITHERROR( MP4BadDataErr )
	...
	for( i = 0, entry = trun->entries; i < sampleCount; i++, entry++ ) {
		...
		entry->sample_flags = ( ( padsH == NULL ) ? 0 : ( ( (u8 *)padsH )[i] ) << 17 ) + ( ( syncSamplesH == NULL ) ? 0 : fragment_difference_sample_flag );
...
static MP4Err addSampleReference( struct MP4MediaInformationAtom *s, u64 dataOffset, u32 sampleCount, MP4Handle durationsH, MP4Handle sizesH, MP4Handle sampleEntryH, MP4Handle decodingOffsetsH, MP4Handle syncSamplesH, MP4Handle padsH )
{
	MP4Err MP4CreateTrackRunAtom( MP4TrackRunAtomPtr * outAtom );
	MP4Err err;
	...
	if( ( self->samples_use_mdat == 0 ) || ( self->samples_use_mdat == 2 ) )
		self->samples_use_mdat = 2;
	else
		BAILWITHERROR( MP4BadDataErr )
	...
	tfhd = (MP4TrackFragmentHeaderAtomPtr)self->tfhd;
	if( tfhd->base_data_offset == 0 )
		tfhd->base_data_offset = dataOffset;
	...
	for( i = 0, entry = trun->entries; i < sampleCount; i++, entry++ ) {
		...
		entry->sample_size = ( ( i >= size_count ) ? sizes[0] : sizes[i] );
		entry->sample_flags = ( ( padsH == NULL ) ? 0 : ( ( (u8 *)padsH )[i] ) << 13 ) + ( ( syncSamplesH == NULL ) ? 0 : fragment_difference_sample_flag );
		entry->sample_composition_time_offset = ( ( decodingOffsetsH == NULL ) ? 0 : ( (u32 *)decodingOffsetsH )[i] );
	}
...
MP4Err mergeSampleAuxiliaryInformation( struct MP4TrackFragmentAtom *self, MP4MediaAtomPtr mdia )
{
	...
	for( i = 0; i < self->saizList->entryCount; i++ ) {
		...
		err = stbl->getSampleAuxiliaryInformation( stbl, ( saizOfTraf->flags & 1 ), saizOfTraf->aux_info_type, saizOfTraf->aux_info_type_parameter, &saizOfStbl, &saioOfStbl );
		if( err )
			goto bail;
		...
		if( err )
			goto bail;
		err = saioOfStbl->mergeOffsets( (MP4AtomPtr)saioOfStbl, (MP4AtomPtr)saioOfTraf, tfhd->base_data_offset );
		if( err )
			goto bail;
	}
...
MP4Err getSampleAuxiliaryInfoFromTrackFragment( struct MP4TrackFragmentAtom *self, u8 isUsingAuxInfoPropertiesFlag, u32 aux_info_type, u32 aux_info_type_parameter, MP4SampleAuxiliaryInformationSizesAtomPtr *saizOut, MP4SampleAuxiliaryInformationOffsetsAtomPtr *saioOut )
{
	...
	*saioOut = NULL;
	...
	for( i = 0; i < self->saizList->entryCount; i++ ) {
		...
		if( ( ( saizExisting->flags & 1 ) == isUsingAuxInfoPropertiesFlag ) && ( saizExisting->aux_info_type == aux_info_type ) && ( saizExisting->aux_info_type_parameter == aux_info_type_parameter ) ) {
			*saizOut = saizExisting;
		}

		if( ( ( saioExisting->flags & 1 ) == isUsingAuxInfoPropertiesFlag ) && ( saioExisting->aux_info_type == aux_info_type ) && ( saioExisting->aux_info_type_parameter == aux_info_type_parameter ) ) {
	...
	if( !theGroup ) {
		...
		for( i = 0; i < atomListSize; i++ ) {
			...
			if( err )
				goto bail;
			if( trun )
				fragment_sample_count += trun->samplecount;
		}
...
static MP4Err setSampleDependency( struct MP4MediaInformationAtom *s, s32 sample_index, MP4Handle dependencies )
{
	...
	if( sample_index < 0 )
		sample_index = fragment_sample_count + sample_index;

	err = MP4GetHandleSize( dependencies, &count );
	if( err )
		goto bail;
	if( ( sample_index + count ) > fragment_sample_count )
		BAILWITHERROR( MP4BadParamErr );
		...
		if( trun ) {
			for( j = 0; ( j < trun->samplecount ) && ( dep_pos < count ); j++, fragment_pos++ ) {
				if( ( fragment_pos >= 0 ) && ( dep_pos < count ) ) {
					( trun->entries )[j].sample_flags = ( ( trun->entries )[j].sample_flags & ~( 0xFF << 20 ) ) | ( dependency[dep_pos++] << 20 );
				}
			}
		}
	}
