...
ISO_EXTERN( ISOErr )
ISONewFileMeta( ISOMovie theMovie, u32 metaType, ISOMeta *outMeta )
{
	...
	if( moov->meta != NULL ) {
		...
		if( moov->meco == NULL ) {
			err = ISOCreateAdditionalMetaDataContainerAtom( (ISOAdditionalMetaDataContainerAtomPtr *)&moov->meco );
			if( err )
				goto bail;
		}
...
ISO_EXTERN( ISOErr )
ISONewMovieMeta( ISOMovie theMovie, u32 metaType, ISOMeta *outMeta )
{
	ISOMetaAtomPtr                        myMeta;
	ISOAdditionalMetaDataContainerAtomPtr mecoPtr;
	...
	if( moov->meta != NULL ) {
		...
		if( moov->meco == NULL ) {
			MP4AtomPtr tempMeco;
			err = ISOCreateAdditionalMetaDataContainerAtom( (ISOAdditionalMetaDataContainerAtomPtr *)&tempMeco );
			if( err )
				goto bail;
...
ISO_EXTERN( ISOErr )
ISONewTrackMeta( ISOTrack theTrack, u32 metaType, ISOMeta *outMeta )
{
	ISOMetaAtomPtr                        myMeta;
...
ISOGetMetaBoxRelation( ISOMeta first_meta, ISOMeta second_meta, u8 *relation_type )
{
	...
	*relation_type = 0;
	...
	for( i = 0; i < meco->relationList->entryCount; i++ ) {
		...
		if( err )
			goto bail;
		if( ( mere->first_metabox_handler_type == firstMeta->type ) && ( mere->second_metabox_handler_type == secondMeta->type ) ) {
	...
	*item = NULL;
	...
	if( iinf->atomList ) {
		...
		for( i = 0; i < item_total; i++ ) {
			...
			if( ( exact_case && ( strcmp( name, infe->item_name ) == 0 ) ) || ( !exact_case && ( strcasecmp( name, infe->item_name ) == 0 ) ) )
	...
	if( iinf->atomList ) {
		...
		for( i = 0; i < item_total; i++ ) {
			...
			if( infe->item_ID == myItem->item_ID ) {
				...
				if( protection_index )
					*protection_index = infe->protection_index;
				err = MP4NoErr;
				goto bail;
			}
		}
...
bail:
	if( schm )
		schm->destroy( (MP4AtomPtr)schm );
	if( schi )
		schi->destroy( (MP4AtomPtr)schi );
	if( sch_url_copy )
		free( sch_url_copy );
