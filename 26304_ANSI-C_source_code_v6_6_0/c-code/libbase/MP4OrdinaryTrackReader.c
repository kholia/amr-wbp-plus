...
static MP4Err getNextAccessUnit( struct MP4TrackReaderStruct *self, MP4Handle outAccessUnit, u32 *outSize, u32 *outSampleFlags, s32 *outCTS, s32 *outDTS, u32 *outDuration, u8 *outPad )
{
	MP4Err err;
	u32    sampleFlags;
	u64    sampleDTS;
	...
	*outDTS = ( u32 )( sampleDTS - self->segmentMediaTime + self->segmentBeginTime );
	...
	if( ( self->elst ) && ( ( duration + *outDTS ) >= self->segmentEndTime ) ) {
		if( outDuration )
			*outDuration = ( u32 )( self->segmentEndTime - *outDTS );
...
static MP4Err getNextPacket( MP4TrackReaderPtr self, MP4Handle outPacket, u32 *outSize )
{
	...
	err = getNextAccessUnit( self, self->sampleH, &sampleSize, &sampleFlags, &packetCTS, &packetDTS, 0, NULL );
	if( err )
		goto bail;
