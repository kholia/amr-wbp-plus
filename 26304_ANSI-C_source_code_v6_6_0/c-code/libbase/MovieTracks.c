...
MP4_EXTERN( MP4Err )
MP4AddTrackReference( MP4Track theTrack, MP4Track dependsOn, u32 dependencyType, u32 *outReferenceIndex )
{
	MP4Err err;
	u32    dependsOnID;
...
MP4_EXTERN( MP4Err )
MP4GetSubSampleInformationEntryFromTrack( MP4Track theTrack, u32 *flags, u32 *entry_count, u32 **sample_delta, u32 **subsample_count, u32 ***subsample_size_array, u32 ***subsample_priority_array, u32 ***subsample_discardable_array )
{
	MP4Err                         err;
	MP4TrackAtomPtr                trak;
	MP4MediaAtomPtr                mdia;
...
MP4_EXTERN( MP4Err )
MP4AddSubSampleInformationEntry( MP4GenericAtom subsample, u32 sample_delta, u32 subsample_count, MP4Handle subsample_size_array, MP4Handle subsample_priority_array, MP4Handle subsample_discardable_array )
{
	MP4Err                         err;
	MP4SubSampleInformationAtomPtr subs;
...
MP4_EXTERN( MP4Err )
MP4NewTrackMedia( MP4Track theTrack, MP4Media *outMedia, u32 mediaType, u32 timeScale, MP4Handle dataReference )
{
	MP4Err          err;
	MP4TrackAtomPtr trackAtom;
	trackAtom = (MP4TrackAtomPtr)theTrack;
...
MP4_EXTERN( MP4Err )
MP4InsertMediaIntoTrack( MP4Track track, s32 trackStartTime, s32 mediaStartTime, u64 mediaDuration, s32 mediaRate )
{
	MP4Err          err;
	MP4TrackAtomPtr trak;
	MP4MediaAtomPtr mdia;
		...
		*outMediaTime = mediaTime;
		...
		if( editCount == 0 ) {
			...
			*outMediaTime = mediaTime;
			...
			u64 prior;
			u64 next;
			err = elst->getTimeAndRate( (MP4AtomPtr)elst, inTrackTime, movieTimeScale, mediaTimeScale, &mediaTime, &mediaRate, &prior, &next );
			if( err )
				goto bail;
			*outMediaTime = mediaTime;
		}
