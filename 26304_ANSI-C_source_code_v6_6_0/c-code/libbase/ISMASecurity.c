...
{
	return ISMATransformSampleEntrySalt( newTrackFlags, insampleEntryH, selective_encryption, key_indicator_length, IV_length, kms_URL, 0, outsampleEntryH );
}
...
bail:
	...
	if( kms )
		kms->destroy( (MP4AtomPtr)kms );
	if( fmt )
		fmt->destroy( (MP4AtomPtr)fmt );
	if( slt )
		slt->destroy( (MP4AtomPtr)slt );
    ...
    MP4Handle                             outsampleEntryH )
{
	...
	if( ( entry->type != MP4EncAudioSampleEntryAtomType ) && ( entry->type != MP4EncVisualSampleEntryAtomType ) ) {
		err = MP4NotImplementedErr;
		goto bail;
	}
