 ...
 *    void
 */
static void E_DTX_isf_history_aver( Float32 isf_old[], Word16 indices[], Float32 isf_aver[] )
{
	Float32 isf_tmp[2 * M];
	Float32 tmp;
	Word16  i, j, k;
...
void E_DTX_tx_handler( E_DTX_State *st, Word32 vad_flag, Word16 *usedMode )
{
	...
	if( vad_flag != 0 ) {
		...
		if( st->mem_dtx_hangover_count == 0 ) { /* out of decoder analysis hangover  */
			st->mem_dec_ana_elapsed_count = 0;
			*usedMode = MRDTX;
		}
		else { /* in possible analysis hangover */
 ...
 *    signal level
 */
static Float32 E_DTX_level_calculation( Float32 data[], Float32 *sub_level, Word16 count1, Word16 count2, Word16 ind_m, Word16 ind_a, Float32 scale )
{
 ...
 */
static Word16 E_DTX_hangover_addition( E_DTX_Vad_State *st, Word16 low_power, Word16 hang_len, Word16 burst_len )
{
	/*
