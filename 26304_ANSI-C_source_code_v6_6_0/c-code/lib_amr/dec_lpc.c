 ...
 *    void
 */
void D_LPC_isf_2s3s_decode( Word16 *indice, Word16 *isf_q, Word16 *past_isfq, Word16 *isfold, Word16 *isf_buf, Word16 bfi )
{
	Word32 ref_isf[M];
	Word32 L_tmp, i, j;
	Word16 tmp;
 ...
 *    void
 */
void D_LPC_isf_2s5s_decode( Word16 *indice, Word16 *isf_q, Word16 *past_isfq, Word16 *isfold, Word16 *isf_buf, Word16 bfi )
{
	Word32 ref_isf[M];
	Word32 i, j, L_tmp;
	Word16 tmp;
 ...
 *    void
 */
void D_LPC_int_isp_find( Word16 isp_old[], Word16 isp_new[], const Word16 frac[], Word16 Az[] )
{
...
void D_LPC_isf_extrapolation( Word16 HfIsf[] )
{
	...
	tmp = ( tmp * 5461 ) >> 15;
	tmp = tmp + 20390;
	if( tmp > 19456 ) { /* Maximum value of ISF should be at most 7600 Hz */
		tmp = 19456;
	}
	tmp = tmp - HfIsf[M - 2];
