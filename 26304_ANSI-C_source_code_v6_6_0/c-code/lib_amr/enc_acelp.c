 ...
 *    void
 */
static void E_ACELP_h_vec_corr1( Float32 h[], Float32 vec[], UWord8 track, Float32 sign[], Float32 ( *rrixix )[16], Float32 cor[], Word32 dn2_pos[], Word32 nb_pulse )
{
	Word16   i, j;
	Word32   dn;
	Word32 * dn2;
...
static void E_ACELP_h_vec_corr2( Float32 h[], Float32 vec[], UWord8 track, Float32 sign[], Float32 ( *rrixix )[16], Float32 cor[] )
{
	Word32   i, j;
	Float32 *p0;
	Float32  s;
 ...
 *    void
 */
static void E_ACELP_2pulse_search( Word32 nb_pos_ix, UWord8 track_x, UWord8 track_y, Float32 *ps, Float32 *alp, Word32 *ix, Word32 *iy, Float32 dn[], Word32 *dn2, Float32 cor_x[], Float32 cor_y[], Float32 ( *rrixiy )[256] )
{
 ...
 *    (3*N)+1 bits
 */
static Word32 E_ACELP_quant_3p_3N1( Word32 pos1, Word32 pos2, Word32 pos3, Word32 N )
{
	Word32 nb_pos;
	Word32 index;
	nb_pos = ( 1 << ( N - 1 ) );
 ...
 *    (4*N)+1 bits
 */
static Word32 E_ACELP_quant_4p_4N1( Word32 pos1, Word32 pos2, Word32 pos3, Word32 pos4, Word32 N )
{
	Word32 nb_pos;
	Word32 index;
	nb_pos = ( 1 << ( N - 1 ) );
...
static Word32 E_ACELP_quant_5p_5N( Word32 pos[], Word32 N )
{
	...
	case 0:
		index = 1 << ( ( 5 * N ) - 1 );
		index += E_ACELP_quant_3p_3N1( posB[0], posB[1], posB[2], n_1 ) << ( ( 2 * N ) + 1 );
	...
	case 2:
		...
		break;
	case 3:
		index = E_ACELP_quant_3p_3N1( posA[0], posA[1], posA[2], n_1 ) << ( ( 2 * N ) + 1 );
...
void E_ACELP_4t( Float32 dn[], Float32 cn[], Float32 H[], Word16 code[], Float32 y[], Word32 nbbits, Word16 mode, Word32 _index[] )
{
    ...
    */
	...
	for( k = 0; k < nbiter; k++ ) {
...
#endif
		{
			...
			alp = rrixix[ipos[0]][ind[0] >> 2] + rrixix[ipos[1]][ind[1] >> 2] + rrixiy[ipos[0]][( ( ind[0] >> 2 ) << 4 ) + ( ind[1] >> 2 )];
			if( sign[ind[0]] < 0.0 ) {
				p0 = h_inv - ind[0];
			}
			else {
		...
		for( j = pos, st = 0; j < nb_pulse; j += 2, st++ ) {
          ...
          */
			E_ACELP_h_vec_corr1( h, vec, ipos[j], sign, rrixix, cor_x, dn2_pos, nbpos[st] );
			E_ACELP_h_vec_corr2( h, vec, ipos[j + 1], sign, rrixix, cor_y );
			/*
          * Find best positions of 2 pulses.
          */
			E_ACELP_2pulse_search( nbpos[st], ipos[j], ipos[j + 1], &ps, &alp, &ind[j], &ind[j + 1], dn, dn2_pos, cor_x, cor_y, rrixiy );
			if( j < ( nb_pulse - 2 ) ) {
				p0 = h - ind[j];
				if( sign[ind[j]] < 0.0 ) {
					p0 = h_inv - ind[j];
   ...
   */
	...
	if( nbbits == 20 ) {
		...
		for( track = 0; track < ( 4 - 2 ); track++ ) {
			k = track * NPMAXPT;
			_index[track] = E_ACELP_quant_3p_3N1( ind[k], ind[k + 1], ind[k + 2], 4 );
		}
		for( track = 2; track < 4; track++ ) {
			k = track * NPMAXPT;
			_index[track] = E_ACELP_quant_2p_2N1( ind[k], ind[k + 1], 4 );
 ...
 *    index of quantizer
 */
Word32 E_ACELP_gains_quantise( Word16 code[], Word32 nbits, Float32 f_gain_pit, Word16 *gain_pit, Word32 *gain_code, Float32 *coeff, Word32 gp_clip, Word16 *past_qua_en )
{
