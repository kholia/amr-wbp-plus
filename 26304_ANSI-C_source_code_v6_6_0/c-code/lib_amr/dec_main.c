 ...
 *    0 if successful
 */
Word32 D_MAIN_decode( Word16 mode, Word16 prms[], Word16 synth16k[], void *spd_state, UWord8 frame_type )
{
	...
	if( newDTXState != SPEECH ) /* CNG mode */
	{
       ...
       */
		...
		for( i_subfr = 0; i_subfr < L_FRAME; i_subfr += L_SUBFR ) {
			...
			D_UTIL_dec_synthesis( Aq, &exc2[i_subfr], 0, &synth16k[i_subfr * 5 / 4], (Word16)1, HfIsf, mode, newDTXState, bfi, st );
		}
    ...
    */
	...
	if( mode <= MODE_7k ) {
		...
		ind[5] = *prms++;
		ind[6] = *prms++;
		D_LPC_isf_2s5s_decode( ind, isf, st->mem_isf_q, st->mem_isf, st->mem_isf_buf, bfi );
	}
	/* Convert ISFs to the cosine domain */
	D_LPC_isf_isp_conversion( isf, ispnew, M );
    ...
    */
	...
	for( i_subfr = 0; i_subfr < L_FRAME; i_subfr += L_SUBFR ) {
       ...
       */
		if( pit_flag == 0 ) {
			if( mode <= MODE_9k ) {
				...
				if( index < ( ( PIT_FR2 - PIT_MIN ) * 4 ) ) {
					...
					T0 = index + ( PIT_FR1_9b - ( ( PIT_FR2 - PIT_MIN ) * 4 ) - ( ( PIT_FR1_9b - PIT_FR2 ) * 2 ) );
					T0_frac = 0;
				}
			}
		...
		{
			D_GAIN_lag_concealment( &( st->mem_gain[17] ), st->mem_lag, &T0, &( st->mem_T0 ), &( st->mem_seed3 ), unusable_frame );
			T0_frac = 0;
		}
		/*
       ...
       */
		...
		if( mode <= MODE_9k ) {
			D_GAIN_decode( index, 6, code, &gain_pit, &L_gain_code, bfi, st->mem_bfi, st->mem_state, unusable_frame, st->mem_vad_hist, st->mem_gain );
		}
		else {
			D_GAIN_decode( index, 7, code, &gain_pit, &L_gain_code, bfi, st->mem_bfi, st->mem_state, unusable_frame, st->mem_vad_hist, st->mem_gain );
		}
