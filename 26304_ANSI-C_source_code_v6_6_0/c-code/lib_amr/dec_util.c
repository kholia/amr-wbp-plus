 ...
 *    void
 */
static void D_UTIL_normalised_log2( Word32 L_x, Word16 exp, Word16 *exponent, Word16 *fraction )
{
	Word32 i, a, tmp;
	Word32 L_y;
	if( L_x <= 0 ) {
 ...
 *    void
 */
void D_UTIL_synthesis_32( Word16 a[], Word16 m, Word16 exc[], Word16 Qnew, Word16 sig_hi[], Word16 sig_lo[], Word16 lg )
{
	Word32 i, j, a0;
	Word32 tmp, tmp2;
 ...
 */
Word16 D_UTIL_interpol( Word16 *x, Word16 const *fir, Word16 frac, Word16 resol, Word16 nb_coef )
{
	Word32 i, k;
	Word32 sum;
	x = x - nb_coef + 1;
 ...
 *    void
 */
void D_UTIL_dec_synthesis( Word16 Aq[], Word16 exc[], Word16 Q_new, Word16 synth16k[], Word16 prms, Word16 HfIsf[], Word16 mode, Word16 newDTXState, Word16 bfi, Decoder_State *st )
{
	Word32  tmp, i;
	Word16  exp;
    ...
    */
	...
	D_UTIL_deemph_32( synth_hi + M, synth_lo + M, synth_new, PREEMPH_FAC, L_SUBFR, &( st->mem_deemph ) );
	D_UTIL_hp50_12k8( synth_out, L_SUBFR, st->mem_sig_out );
	D_UTIL_oversamp_16k( synth_out, L_SUBFR, synth16k, st->mem_oversamp );
	if( st->ramp_state > 0 ) {
		st->ramp_state--;
		gain_hf_plus = st->gain_hf_plus * gain_hf_ramp[st->ramp_state] * (float)pow( 2.0f, -1.0f * (float)Q_new );
		for( i = 0; i < L_SUBFR; i++ ) {
			f_exc[i] = gain_hf_plus * (float)exc[i];
		}
		soft_exc_hf( f_exc, &( st->lp_amp_hf ) );
		E_UTIL_synthesisPlus( st->lpc_hf_plus, 8, f_exc, f_synth, L_SUBFR, st->mem_syn_hf_plus, 1 );
	...
	memcpy( st->mem_syn_out, synth_buff + L_SUBFR, ( PIT_MAX + L_SUBFR ) * sizeof( Word16 ) );
	return;
}
/*
 * D_UTIL_preemph
