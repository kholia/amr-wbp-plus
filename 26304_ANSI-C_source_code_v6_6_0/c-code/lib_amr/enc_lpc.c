 ...
 *    void
 */
void E_LPC_int_isp_find( Word16 isp_old[], Word16 isp_new[], const Word16 frac[], Word16 Az[] )
{
...
void E_LPC_f_int_isp_find( Float32 isp_old[], Float32 isp_new[], Float32 a[], Word32 nb_subfr, Word32 m )
{
...
void E_LPC_isf_2s5s_decode( Word32 *indice, Word16 *isf_q, Word16 *past_isfq )
{
	...
	for( i = 0; i < 3; i++ ) {
		isf_q[i] = ( Word16 )( isf_q[i] + ( Word32 )( ( E_ROM_dico21_isf[indice[2] * 3 + i] * 2.56F ) + 0.5F ) );
	}
	for( i = 0; i < 3; i++ ) {
		isf_q[i + 3] = ( Word16 )( isf_q[i + 3] + ( Word32 )( ( E_ROM_dico22_isf[indice[3] * 3 + i] * 2.56F ) + 0.5F ) );
	}
	for( i = 0; i < 3; i++ ) {
		isf_q[i + 6] = ( Word16 )( isf_q[i + 6] + ( Word32 )( ( E_ROM_dico23_isf[indice[4] * 3 + i] * 2.56F ) + 0.5F ) );
	}
	for( i = 0; i < 3; i++ ) {
		isf_q[i + 9] = ( Word16 )( isf_q[i + 9] + ( Word32 )( ( E_ROM_dico24_isf[indice[5] * 3 + i] * 2.56F ) + 0.5F ) );
	}
	for( i = 0; i < 4; i++ ) {
		isf_q[i + 12] = ( Word16 )( isf_q[i + 12] + ( Word32 )( ( E_ROM_dico25_isf[indice[6] * 4 + i] * 2.56F ) + 0.5F ) );
	}
	for( i = 0; i < ORDER; i++ ) {
		tmp = isf_q[i];
		isf_q[i] = ( Word16 )( tmp + E_ROM_mean_isf[i] );
 ...
 *    quantisation index
 */
Word16 E_LPC_isf_sub_vq( Float32 *x, const Float32 *E_ROM_dico, Word32 dim, Word32 E_ROM_dico_size, Float32 *distance )
{
 ...
 *    void
 */
static void E_LPC_stage1_isf_vq( Float32 *x, const Float32 *E_ROM_dico, Word32 dim, Word32 E_ROM_dico_size, Word32 *index, Word32 surv )
{
 ...
 *    void
 */
void E_LPC_isf_2s3s_quantise( Float32 *isf1, Word16 *isf_q, Word16 *past_isfq, Word32 *indice, Word32 nb_surv )
{
	...
	Word32  i, k;
	for( i = 0; i < ORDER; i++ ) {
		isf[i] = ( Float32 )( ( isf1[i] - E_ROM_f_mean_isf[i] ) - F_MU * past_isfq[i] * 0.390625F );
	}
	...
	for( k = 0; k < nb_surv; k++ ) {
		...
		tmp_ind[0] = E_LPC_isf_sub_vq( &isf_stage2[0], E_ROM_dico21_isf_36b, 5, SIZE_BK21_36b, &min_err );
		temp = min_err;
		tmp_ind[1] = E_LPC_isf_sub_vq( &isf_stage2[5], E_ROM_dico22_isf_36b, 4, SIZE_BK22_36b, &min_err );
		temp += min_err;
		if( temp < distance ) {
			distance = temp;
			indice[0] = surv1[k];
	...
	for( k = 0; k < nb_surv; k++ ) {
		...
		tmp_ind[0] = E_LPC_isf_sub_vq( &isf_stage2[0], E_ROM_dico23_isf_36b, 7, SIZE_BK23_36b, &min_err );
		temp = min_err;
		if( temp < distance ) {
			distance = temp;
			indice[1] = surv1[k];
 ...
 *    void
 */
void E_LPC_isf_2s5s_quantise( Float32 *isf1, Word16 *isf_q, Word16 *past_isfq, Word32 *indice, Word32 nb_surv )
{
	Float32 isf[ORDER], isf_stage2[ORDER];
	Float32 temp, min_err, distance;
	Word32  surv1[N_SURV_MAX]; /* indices of survivors from 1st stage */
	...
	for( k = 0; k < nb_surv; k++ ) {
		...
		tmp_ind[0] = E_LPC_isf_sub_vq( &isf_stage2[0], E_ROM_dico21_isf, 3, SIZE_BK21, &min_err );
		temp = min_err;
		tmp_ind[1] = E_LPC_isf_sub_vq( &isf_stage2[3], E_ROM_dico22_isf, 3, SIZE_BK22, &min_err );
		temp += min_err;
		tmp_ind[2] = E_LPC_isf_sub_vq( &isf_stage2[6], E_ROM_dico23_isf, 3, SIZE_BK23, &min_err );
		temp += min_err;
		if( temp < distance ) {
			distance = temp;
			indice[0] = surv1[k];
	...
	for( k = 0; k < nb_surv; k++ ) {
		...
		tmp_ind[0] = E_LPC_isf_sub_vq( &isf_stage2[0], E_ROM_dico24_isf, 3, SIZE_BK24, &min_err );
		temp = min_err;
		tmp_ind[1] = E_LPC_isf_sub_vq( &isf_stage2[3], E_ROM_dico25_isf, 4, SIZE_BK25, &min_err );
		temp += min_err;
		if( temp < distance ) {
			distance = temp;
			indice[1] = surv1[k];
